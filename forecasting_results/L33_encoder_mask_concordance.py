...
def train_mask_concordance_model(train_data, val_data, model, epochs, lr, patience, checkpoint_path, inner_batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        for case_report in train_data:
            for batch in case_report:
                ...
                labels = torch.tensor(
                    [1 if item['label'] == model.before_token_id else 0 for item in batch],
                ...
                mask_logits = mask_logits[:, model.target_token_ids]  # Only keep "before" and "after" logits
        ...
        avg_val_loss, concordance_index = evaluate_model_mask_concordance(model, val_data)
...
def evaluate_model_mask_concordance(model, val_data, inner_batch_size=32):
    model.eval()
    total_loss = 0
    ...
    with torch.no_grad():
        for case_report in val_data:
            ...
            for batch in case_report:
                ...
                labels = torch.tensor([1 if item['label'] == model.before_token_id else 0 for item in batch],
                    dtype=torch.long, device=device)
                ...
                preds = torch.argmax(mask_logits, dim=1).cpu().numpy()
...
def prepare_dataset(timeseries_data, K=8, H=24, tokenizer=None, max_length=512, inner_batch_size=32):
        ...
        for timeseries_num, (history, next_events) in enumerate(batch):
            ...
            for (e1, t1), (e2, t2) in itertools.permutations(shuffled_events, 2):
                if t1 != t2:
                    ...
                    label_text = "before" if t1 < t2 else "after"
                    history_text = " [SEP] ".join([f'({e[1]}) : '+e[0] for e in history])
                    input_text = f"[CLS] {history_text} [SEP] {e1} [MASK] {e2} [SEP]"  
                    ...
                    concordance_data.append({'inputs': {k: v.squeeze(0) for k, v in tokenized_input.items()},'label': label_token_id, 'timeseries_num': timeseries_num})
                    

            if len(concordance_data) > 0:
                # Batch inner data like before
                batched_case_report = [concordance_data[i:i + inner_batch_size]
 ...
 # **Updated Model for MLM prediction of "before" or "after"**
class MaskedEventClassifier(nn.Module):
    def __init__(self, model_name, tokenizer, cache_dir):
        ...
        self.before_token_id = tokenizer.convert_tokens_to_ids("before")
        self.after_token_id = tokenizer.convert_tokens_to_ids("after")
        self.target_token_ids = torch.tensor([self.before_token_id, self.after_token_id], dtype=torch.long).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path_mask_concordance', type=str, default='best_model_mask_concordance.pth', help='Checkpoint path for concordance')
    ...
    parser.add_argument('--test_results_text_file', type=str, default='test_concordance_mask.txt', help='File to save test results')
    ...
    if not args.do_zero_shot:
        ...
        #     checkpoint_path=args.checkpoint_path_mask_concordance,
        #     inner_batch_size=args.batch_size)

        #Load the best model

        model.load_state_dict(torch.load(args.checkpoint_path_mask_concordance))

        #checkpoint = torch.load(args.checkpoint_path_mask_concordance, weights_only=False)
        #model.load_state_dict(checkpoint['model_state_dict'])
        #best_threshold = checkpoint['best_threshold']


    print('Evaluating concordance on test set')
    ...
    # Evaluate on validation set
    _, concordance_index_score = evaluate_model_mask_concordance(model, concordance_data_test)
