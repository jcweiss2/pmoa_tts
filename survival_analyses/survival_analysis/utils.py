...
def get_integrated_brier_score(survival_train, survival_test, surv, times):
    ...
    valid_non_nan_patients = ~filtered_surv.isna().any(axis=0)
    filtered_surv_clean = filtered_surv.loc[:, valid_non_nan_patients]
    survival_test_clean = survival_test[valid_non_nan_patients]
    
    # return integrated_brier_score(survival_train, survival_test, filtered_surv.T, times_exclusive)
    return integrated_brier_score(survival_train, survival_test_clean, filtered_surv_clean.T, times_exclusive)
