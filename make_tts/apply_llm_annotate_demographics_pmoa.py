...
def run(args):
    ...
    out_dir_dx = out_dir + "demographics/"  # unused (forgot to point here)
    ...
    if True:
        ...
        for idx, case in enumerate(case_filenames):
            # if idx < 18:
            #     continue
            if filter_filenames is not None and os.path.splitext(case)[0] not in filter_filenames:
                print(f'filter_filenames present and {case} not in it')
                continue
            if idx % 5 == 0:
                out_dir_files = os.listdir(out_dir_dx)  # check at beginning for files completed
